...
public class KinectManager : MonoBehaviour
{
    ...
    public bool user_inout = constant.userOut;
    ...
    class constant
    {
        public const bool userIn = true;
        public const bool userOut = false;
    }
    ...
    private void UpdateKinectStreams()
    {
        if (kinectInitialized)
        {
            ...
            bLimitedUsers = showTrackedUsersOnly && ((maxTrackedUsers < 6 ) ||
            ...
            if (computeUserMap != UserMapType.None)
            {
                ...
                sensorData.newDepthImage = KinectInterop.PollDepthFrame(sensorData, computeUserMap, bLimitedUsers, dictUserIdToIndex.Values);
    ...
    void Update()
    {
        // Debug.Log("avatarControllers.Count : " + avatarControllers.Count);
        if (kinectInitialized)
        {
    ...
    public void ProcessBodyFrameData()
    {
        int[] b= new int[2];
        
        List<Int64> addedUsers = new List<Int64>(); // 새로 발견 유저
        List<int> addedIndexes = new List<int>(); // 새로 발견 유저 인덱스

        List<Int64> lostUsers = new List<Int64>(); // 사라진 유저
    ...
    public void RefreshAvatarUserIds()
    {
        ...
        for (int i = 0; i < avatarControllers.Count; i++)
        {
            ...
            Debug.Log("oldUserId : " + oldUserId);
            Debug.Log("newUserId : " + newUserId);
            Debug.Log("avatar.playerIndex : " + avatar.playerIndex);
    ...
    protected virtual void RemoveUser(Int64 userId)
    {
        ...
        // aUserIndexIds 한칸싞 당김
        if(aUserIndexIds[0]==0)
        {
            for(int i =0; i< KinectInterop.Constants.MaxBodyCount; i++)
            {
                if (i+1 == KinectInterop.Constants.MaxBodyCount)
                    continue;
                aUserIndexIds[i] = aUserIndexIds[i + 1];
